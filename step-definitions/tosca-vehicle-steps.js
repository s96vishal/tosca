...
module.exports=async function(){
    ...
    driver.takeScreenshot().then((image,err)=>{
        require('fs').writeFile('./reports/screenshots/screen1.png',image,'base64',function(err){console.log(err);})
    })
    ...
    driver.takeScreenshot().then((image,err)=>{
        require('fs').writeFile('./reports/screenshots/screen2.png',image,'base64',function(err){console.log(err);})
    })
    await this.When("I enter details and click next",function(){
        ...
        driver.takeScreenshot().then((image,err)=>{
            require('fs').writeFile('./reports/screenshots/screen3.png',image,'base64',function(err){console.log(err);})
        })
    ...
    this.When('I enter product data and click next',function(){
        ...
        driver.findElement(by.id('startdate')).sendKeys("09/05/2020");
    ...
    this.Then('I should see price options',function(){
        return driver.wait(until.elementsLocated(by.id('priceTable')),20000);
    })

    await this.When('I select pricing option and click next',function(){
        driver.wait(until.elementsLocated(by.xpath('//*[@id="priceTable"]/tfoot/tr/th[2]/label[1]/span')),40000).then(()=>{
  ...
  this.Then('I should see',function(){
    ...
    this.When('I enter send quote data and click send',function(){
        driver.wait(until.elementsLocated(by.id('email')),30000).then(()=>{
